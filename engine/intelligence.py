...
1. "type" — тип обращения, СТРОГО одно из:
   ...
   Решающие границы:
   - Если клиент ТРЕБУЕТ вернуть деньги → "Претензия" (не "Жалоба")
   - Если клиент обнаружил НЕИЗВЕСТНЫЕ операции → "Мошеннические действия" (не "Претензия")
   - Если текст не связан с финансовыми услугами → "Спам"
...
5. "normalized_address" — извлечь город и улицу из текста (если упоминаются). Формат: "Город, улица" или "Unknown" если нет адреса.

6. "summary" — краткое резюме обращения (1-2 предложения на русском) + рекомендуемые следующие действия для менеджера.
