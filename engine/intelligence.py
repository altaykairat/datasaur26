...
1. "type" — тип обращения, СТРОГО одно из:
   ...
   - Если в обращении есть признаки мошенничества/несанкционированных операций → ВСЕГДА "Мошеннические действия" (даже если есть жалоба/требование).
   - Иначе если клиент требует вернуть деньги/угрожает судом → "Претензия" (не "Жалоба").
   - Иначе если основная проблема — техническая недоступность (ошибки, не входит, зависает) без упора на возврат → "Неработоспособность приложения".
   - Иначе если текст не связан с финансовыми услугами → "Спам".
...
class IntelligenceEngine:
    ...
    def _parse_response(self, raw: str) -> dict:
        ...
        if result["language"] not in valid_languages:
            ...
            # Применяем правило: если невозможно определить язык -> RU + confidence < 0.5
