...
1. "type" — тип обращения, СТРОГО одно из:
   ...
   1. "Мошеннические действия" — если есть признаки мошенничества/несанкционированных операций или подозрительные ссылки, требующие ввода данных.
   2. "Спам" — если сообщение содержит только рекламу или нерелевантные ссылки (например, казино, ставки), даже если есть ссылка.
   3. "Претензия" — если есть требование возврата денег/угроза судом.
   4. "Неработоспособность приложения" — если основная проблема — техническая недоступность (ошибка, не входит, зависает) без упора на возврат.
   5. "Смена данных" — запрос на обновление информации.
   6. "Жалоба" — явный негатив или недовольство.
   7. "Консультация" — обычный вопрос (даже если есть ссылка на скриншот или легитимный ресурс).
...
class IntelligenceEngine:
    ...
    def _parse_response(self, raw: str) -> dict:
        ...
        # Validate sentiment with normalization
        s_raw = str(result["sentiment"]).strip().lower()
        if s_raw in ("позитивный", "positive", "pos"):
            result["sentiment"] = "Позитивный"
        elif s_raw in ("негативный", "negative", "neg"):
